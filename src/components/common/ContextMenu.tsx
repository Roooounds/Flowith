...
function SubMenuItem({
  ...
  return (
    <div className="relative" onMouseEnter={onEnter} onMouseLeave={onLeave}>
      ...
      {open && (
        <div
          className="absolute left-full top-0 ml-1 w-40 bg-boss-surface border border-boss-border rounded-lg shadow-2xl py-1 z-[70]"
...
export default function ContextMenu({ x, y, onClose, nodeId, onAddNode }: ContextMenuProps) {
  ...
  if (nodeId) {
    ...
    return (
      <div
        ref={menuRef}
        className="fixed z-[60] w-44 bg-boss-surface border border-boss-border rounded-lg shadow-2xl py-1 overflow-hidden"
      ...
      >
        ...
        <MenuItem onClick={() => { duplicateNode(nodeId); onClose(); }} shortcut="⌘D">
          <svg className="w-3.5 h-3.5 text-boss-accent" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Delete */}
        <MenuItem onClick={() => { removeNode(nodeId); onClose(); }} danger shortcut="⌫">
